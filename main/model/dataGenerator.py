...
def read_image_and_gt(img_files, gt_files, config):
    ...
    for img_name, gt_name in zip(img_files, gt_files):
        ...
        # and store
        imgs[img_idx] = np.asarray(img.reshape(config.IMAGE_HEIGHT, config.IMAGE_WIDTH, config.N_CHANNELS))
        ...
        for i in range(len(bboxes_per_file)):
            ...
            delta[2] = box_w / config.ANCHOR_BOX[aidx][2]
            delta[3] = box_h / config.ANCHOR_BOX[aidx][3]
...
def read_image_and_gt_with_original(img_files, gt_files, config):
    ...
    for img_name, gt_name in zip(img_files, gt_files):
        ...
        v = np.where(v <= 255 - add_v, v + add_v, 255)
        final_hsv = cv2.merge((h, s, v))
        img = cv2.cvtColor(final_hsv, cv2.COLOR_HSV2BGR)  # store original height and width?
        orig_h, orig_w, _ = [float(v) for v in img.shape]
