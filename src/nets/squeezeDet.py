...
class SqueezeDet(ModelSkeleton):
  ...
  def _add_forward_graph(self):
    ...
    fire2 = self._mobile_layerv2('fire2', fire1, oc=depth[1], freeze=False, w_bin=ml_w_bin, a_bin=ml_a_bin,
                                 pool_en=False, min_rng=min_rng, max_rng=max_rng, expansion=1)
    fire3 = self._mobile_layerv2('fire3', fire2, oc=depth[2], freeze=False, w_bin=ml_w_bin, a_bin=ml_a_bin,
                                 min_rng=min_rng, max_rng=max_rng, expansion=1)
  ...
  def _mobile_layerv2(self, layer_name, inputs, oc, stddev=0.01, freeze=False, w_bin=16, a_bin=16, pool_en=True, min_rng=-0.5, max_rng = 0.5, stride = 1, expansion=1):
      ...
      with tf.variable_scope(layer_name,reuse=tf.AUTO_REUSE):
      	...
      	#Depthwise layer
      	ex3x3 = self._conv_layer('dw_conv3x3', exp1x1, filters=int(exp1x1.get_shape()[3]), size=3, stride=1,padding='SAME', stddev=stddev, freeze=freeze, relu=False, w_bin=w_bin, depthwise=True)
      	tf.summary.histogram('dw_before_bn', ex3x3)
      	ex3x3 = self._batch_norm('dw_bn', ex3x3) # <----
      	tf.summary.histogram('dw_before_relu', ex3x3)
      	ex3x3 = self.binary_wrapper(ex3x3, a_bin=a_bin, min_rng=min_rng, max_rng=max_rng) # <---- relu
      	tf.summary.histogram('dw_after_relu', ex3x3)
      	pro1x1 = self._conv_layer('prooconv1x1', ex3x3, filters=oc, size=1, stride=1,padding='SAME', stddev=stddev, freeze=freeze, relu=False, w_bin=w_bin, depthwise=False)
