...
class SqueezeDet(ModelSkeleton):
  ...
  def _add_forward_graph(self,freeze_layers):
    ...
    if False: # Stride 2
        fire1 = self._fire_layer  ('fire1', self.image_input, oc=depth[0], freeze=freeze_layers[0], w_bin=fl_w_bin, a_bin=fl_a_bin, pool_en=False, min_rng=min_rng, max_rng=max_rng, stride=2)
    else: # Max pool
        fire1 = self._fire_layer  ('fire1', self.image_input, oc=depth[0], freeze=freeze_layers[0], w_bin=fl_w_bin, a_bin=fl_a_bin, min_rng=min_rng, max_rng=max_rng, stride=1)
    fire2 = self._mobile_layer('fire2', fire1, oc=depth[1], freeze=freeze_layers[1], w_bin=ml_w_bin, a_bin=ml_a_bin, min_rng=min_rng, max_rng=max_rng)
    if False: # stride 2
        fire3 = self._mobile_layer('fire3', fire2, oc=depth[2], freeze=freeze_layers[2], w_bin=ml_w_bin, a_bin=ml_a_bin, pool_en=False, min_rng=min_rng, max_rng=max_rng, stride=2)
    else:    # Max pool
        fire3 = self._mobile_layer('fire3', fire2, oc=depth[2], freeze=freeze_layers[2], w_bin=ml_w_bin, a_bin=ml_a_bin, pool_en=False, min_rng=min_rng, max_rng=max_rng, stride=1)
    fire4 = self._mobile_layer('fire4', fire3, oc=depth[3], freeze=freeze_layers[3], w_bin=ml_w_bin, a_bin=ml_a_bin, min_rng=min_rng, max_rng=max_rng)
    fire5 = self._mobile_layer('fire5', fire4, oc=depth[4], freeze=freeze_layers[4], w_bin=ml_w_bin, a_bin=ml_a_bin, pool_en=False, min_rng=min_rng, max_rng=max_rng)
    fire6 = self._mobile_layer('fire6', fire5, oc=depth[5], freeze=freeze_layers[5], w_bin=ml_w_bin, a_bin=ml_a_bin, min_rng=min_rng, max_rng=max_rng)
    fire7 = self._mobile_layer('fire7', fire6, oc=depth[6], freeze=freeze_layers[6], w_bin=ml_w_bin, a_bin=ml_a_bin, pool_en=False, min_rng=min_rng, max_rng=max_rng)
    fire_o = fire7
