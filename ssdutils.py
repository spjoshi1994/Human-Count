...
def compute_location(box, anchor):
    ...
    arr[2] = (box.size.w/anchor.size.w)
    arr[3] = (box.size.h/anchor.size.h)
    return arr
...
def decode_location(box, anchor):
    ...
    w = (box[2]) * anchor.size.w
    h = (box[3]) * anchor.size.h
...
def non_maximum_suppression(boxes, overlap_threshold):
    ...
    for box in boxes:
        '''xmin_single = box[1].center.x - box[1].size.w/2
        xmax_single = box[1].center.x + box[1].size.w/2
        ymin_single = box[1].center.y - box[1].size.h/2
        ymax_single = box[1].center.y + box[1].size.h/2
        
        xmin.append(int(xmin_single))
        xmax.append(int(xmax_single))
        ymin.append(int(ymin_single))
        ymax.append(int(ymax_single))
        conf.append(box[0])'''
