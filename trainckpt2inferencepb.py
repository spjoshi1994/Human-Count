...
def parseCkptFolder(fullPathOfFolder,shapeInforNodeName,inputNodeName, outputNodeName):
    ...
    if len(meta_files)==0: #Only frozen model 
        raise ValueError('There should be one meta file in the model directory (%s)' % folderDir)
     
    elif len(meta_files)==1:
        ckptFile = os.path.basename(meta_files[0])
        ...
        raise ValueError('There should not be more than one meta file in the model directory (%s)' % folderDir)  
    inputShapeArray=[] 
...
def demoCKPT2PB(dataPara):  
    ...
    currentFolder= './'+modelFileName.replace('.pbtxt','')
