...
def prop2abs(center, size, imgsize):
    ...
    '''xmin = center.x - size.w/2
    xmax = center.x + size.w/2
    ymin = center.y - size.h/2
    ymax = center.y + size.h/2
...
def draw_box(img, box, color):
    ...
    xmin, xmax, ymin, ymax = prop2abs(box.center, box.size, img_size)
    '''xmin = int((box.center.x/1000)*img_size.w - ((box.size.w/1000)*img_size.w)/2)
    xmax = int( (box.center.x/1000)*img_size.w +(( box.size.w/1000)*img_size.w)/2)
    ymin = int( (box.center.y/1000)*img_size.h -(( box.size.h/1000)*img_size.h)/2)
    ymax = int( (box.center.y/1000)*img_size.h +(( box.size.h/1000)*img_size.h)/2)'''
